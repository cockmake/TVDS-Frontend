...
const columns = [
  ...
  { title: '审核状态', dataIndex: 'manualReviewStatus', key: 'manualReviewStatus',
    customRender:({text})=> text ? '已审核' : '未审核',
    filters:[{text:'已审核',value:true},{text:'未审核',value:false}],
    onFilter:(v,r)=>Boolean(r.manualReviewStatus)===v, width:'120px'},
...
const handleManualReview = () => {
  if (selectedDetectionResult.value) {
    selectedDetectionResult.value.manualReviewStatus = true;
    // 同时更新列表中的数据
    const record = detectionResults.value.find(r => r.resultId === selectedDetectionResult.value.resultId);
    if (record) {
      record.manualReviewStatus = true;
    }
  }
  message.success('人工审核已通过');
};
</script>
    ...
    <div style="display:flex; flex-wrap:wrap; gap:16px;">
      <a-table
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'manualReviewStatus'">
            <a-space style="width:100%; justify-content:center;">
              <a-tag :color="record.manualReviewStatus ? 'blue':'orange'" style="width:100%; text-align:center; line-height:25px;">
                {{ record.manualReviewStatus ? '已审核':'未审核' }}
              </a-tag>
            </a-space>
          </template>
        </template>
  ...
  <a-modal v-model:open="detailModalVisible"
    ...
    <template #footer>
      <div style="display: flex; justify-content: flex-end;">
        <a-button type="primary" @click="handleManualReview">人工审核</a-button>
      </div>
    </template>
    <template #title>
    ...
    <div v-if="selectedDetectionResult" style="height:400px; text-align:center; margin-bottom: 16px;">
    ...
    <a-descriptions
        ...
        bordered
        style="margin-bottom: 16px;">
      <a-descriptions-item label="入站时间">{{ selectedDetectionResult.vehicleCreatedAt }}</a-descriptions-item>
    ...
    <a-descriptions
      ...
      <a-descriptions-item label="开始时间">{{ selectedDetectionResult.taskCreatedAt }}</a-descriptions-item>
      <a-descriptions-item label="完成时间">{{ selectedDetectionResult.taskUpdatedAt }}</a-descriptions-item>
      ...
      <a-descriptions-item label="审核状态">
        <a-tag :color="selectedDetectionResult.manualReviewStatus ? 'blue':'orange'">
          {{ selectedDetectionResult.manualReviewStatus ? '已审核':'未审核' }}
        </a-tag>
      </a-descriptions-item>
